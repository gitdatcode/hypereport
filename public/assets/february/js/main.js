 ...
 */
{
    ...
    const open = (pos) => {
        this.isOpen = true;
        const report = pos + 1;
        document.getElementById('wrap' + report).classList.add('live');
        window.history.pushState('', 'February 2019', baseUrl + 'february#' + report);
        anime({
        ...
        currentBlob.expand().then(() => {
            ...
            let $hypeThis = document.querySelector('.report__hype');
            const hypeReport = hype.find( r => r.id === report);
            $hypeThis.classList.add('live');
            $hypeThis.href = "https://twitter.com/home?status=" + encodeURI("Check out " + hypeReport.first_name + "'s February 2019 Hype Report! https://hype.report/february#" + hypeReport.id);
    ...
    const close = () => {
        ...
        this.isOpen = false;
        window.history.pushState('', 'February 2019', baseUrl + 'february');
        document.querySelector('.live').classList.remove('live');
        document.querySelector('.report__hype').classList.remove('live');
    ...
    if (hashed) document.getElementById('report#' + hash).click();
};